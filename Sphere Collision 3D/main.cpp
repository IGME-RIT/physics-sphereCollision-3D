...
// check if the distance between the centers of the two spheres is less than the sum of their radiuses.
bool is_colliding(Sphere &s1, Sphere &s2)
{
	if (glm::distance(s1.origin, s2.origin) <= (s1.radius+s2.radius))
		return true;
...
void setup()
{
	...
	sphere2.origin = glm::vec3(0.0f, 0.0f, 0.0f);
...
void update()
{
	...
	if (is_colliding(sphere1, sphere2))
	{
		blue = 1.0f;
	}
	else
	...
	translation = glm::translate(sphere2.origin);
	sphere2.MVP = PV * translation;
}
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "Sphere collision in 3 Dimensions", nullptr, nullptr);
	std::cout << "\n This is a collision test between two spheres in 3D.\n\n\n\n\n";
